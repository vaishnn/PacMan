...
class Library(QWidget):
    ...
    def __init__(self, config, parent=None):
        """
        Constructor - Initializes the main components of the library widget.
        Calls helper methods to set up properties, managers, UI, and signals.
        """
        super().__init__(parent)
        # Assuming 'self.config' is accessible, e.g., from a parent widget
    ...
    def _init_program_runner(self):
        """Sets up the Go program runner for listing libraries based on the OS."""
        platform = "win32" if sys.platform.startswith("win") else "darwin"
        self.goExecutable = self.config.get("paths", {}).get("executablePaths", {}).get("library", {}).get(platform, "")
    ...
    def _setup_path_selection_bar(self, parent_layout):
        ...
        margin = self.config.get("application", {}).get("library", {}).get("labelLocation", {}).get("contentMargin", [0, 0, 0, 0])
    ...
    def sortItemsList(self, emit: bool = False):
        ...
        for item in items:
            ...
            uninstallButton.setIcon(svg_to_icon(
                self.config.get("paths", {}).get("images", {}).get("uninstall"),
            ))
