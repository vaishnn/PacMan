...
class PyPIitemDelegate(QStyledItemDelegate):
    ...
    def __init__(self, config: dict, parent = None):
        ...
        self.color_hover = QColor(self.config.get(
            "colors", {}).get("background", {}).get("hover", QColor(255, 255, 255))
        )
        self.text_color = QColor(
            self.config.get("colors", {}).get("text", {}).get("normal", QColor(0, 0, 0))
        )
        self.color_muted = QColor(
            self.config.get("colors", {}).get("text", {}).get("muted", QColor(128, 128, 128))
        )
        self.button_color = QColor(
            self.config.get("colors", {}).get("button", {}).get("install", QColor(0, 128, 255))
        )
        self.button_color_hover = QColor(
            self.config.get("colors", {}).get("button", {}).get("primaryHover", QColor(0, 128, 255))
        )
        self.installPixmap = QPixmap("icons/download.png")
        self.padding = 10
        self._hovered_index = None
        self.roundedCornersRadius = self.config.get("application", {}).get(
            "installer", {}).get("roundedCornersRadius", 0)
    ...
    def paint(self, painter: QPainter, option, index): #type: ignore
        ...
        install_icon = self.config.get("paths", {}).get("images", {}).get("install", "")
        installing_icon = self.config.get("paths", {}).get("images", {}).get("installing", "")
        installed_icon = self.config.get("paths", {}).get("images", {}).get("installed", "")
...
class LibraryListModel(QAbstractListModel):
    ...
    def data(self, index, role = Qt.ItemDataRole.DisplayRole):
        ...
        if role == Qt.ItemDataRole.ToolTipRole:
            description = row_item.get('summary', '').strip()
        ...
        for i, item_data in enumerate(_data):
            if item_data.get('name') == name:
                if not data_dict.get('response') == 200:
                    ...
                    _summary = data_dict.get('summary', '')
                    if not _summary :
                        self._data.remove({'name': name})
                    else:
                        print(_summary)
